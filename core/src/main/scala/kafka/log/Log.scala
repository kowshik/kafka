...
class Log(@volatile var logStartOffset: Long,
   ...
   * @param reason A function that accepts the list of deletable segments and returns a function
   *               that provides the reason for the segment deletion
   * @return The number of segments deleted
   */
  private def deleteOldSegments(predicate: (LogSegment, Option[LogSegment]) => Boolean,
                                reason: Iterable[LogSegment] => () => Unit): Int = {
    ...
    lock synchronized {
      ...
      if (deletable.nonEmpty)
        deleteSegments(deletable, reason(deletable))
      else
        0
    }
  }

  private def deleteSegments(deletable: Iterable[LogSegment], reason: () => Unit): Int = {
  ...
  private def deleteRetentionMsBreachedSegments(): Int = {
    ...
    def logSegmentDeletionReason(toDelete: Iterable[LogSegment]): () => Unit = {
      () => {
        val retentionMs = config.retentionMs
        toDelete.foreach { segment =>
          segment.largestRecordTimestamp match {
            case Some(_) =>
              info(s"Deleting segment $segment due to retention time ${retentionMs}ms breach based on the largest " +
                s"record timestamp in the segment")
            case None =>
              info(s"Deleting segment $segment due to retention time ${retentionMs}ms breach based on the " +
                s"last modified time of the segment")
          }
        }
      }
    }
    deleteOldSegments(shouldDelete, logSegmentDeletionReason)
  }
    ...
    def logSegmentDeletionReason(toDelete: Iterable[LogSegment]): () => Unit = {
      () => {
        ...
        toDelete.foreach { segment =>
          ...
          info(s"Deleting segment $segment due to retention size ${config.retentionSize} breach. Log size " +
            s"after deletion will be $logSize.")
        }
      }
    }
    deleteOldSegments(shouldDelete, logSegmentDeletionReason)
  ...
  private def deleteLogStartOffsetBreachedSegments(): Int = {
    ...
    def logSegmentDeletionReason(toDelete: Iterable[LogSegment]): () => Unit = {
      () => {
        info(s"Deleting segments due to log start offset $logStartOffset breach: ${toDelete.mkString(",")}")
      }
    }
    deleteOldSegments(shouldDelete, logSegmentDeletionReason)
  }
  ...
  private[log] def replaceSegments(newSegments: Seq[LogSegment], oldSegments: Seq[LogSegment], isRecoveredSwapFile: Boolean = false): Unit = {
    lock synchronized {
      ...
      val deletedSegments = Log.replaceSegments(localLog.segments, newSegments, oldSegments, dir, topicPartition,
        config, scheduler, logDirFailureChannel, isRecoveredSwapFile)
      deleteProducerSnapshotAsync(deletedSegments)
    }
  }
  ...
  private[log] def splitOverflowedSegment(segment: LogSegment): List[LogSegment] = lock synchronized {
    val result = Log.splitOverflowedSegment(segment, localLog.segments, dir, topicPartition, config, scheduler, logDirFailureChannel, producerStateManager)
...
object Log extends Logging {
  ...
  private[log] def splitOverflowedSegment(segment: LogSegment,
    ...
    LocalLog.splitOverflowedSegment(segment, existingSegments, dir, topicPartition, config, scheduler, logDirFailureChannel, producerStateManager)
  }
  ...
  private[log] def deleteSegmentFiles(segmentsToDelete: Iterable[LogSegment],
    ...
    LocalLog.deleteSegmentFiles(segmentsToDelete, asyncDelete, dir, topicPartition, config, scheduler, logDirFailureChannel)
  }
